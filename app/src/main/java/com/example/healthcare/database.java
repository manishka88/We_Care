...
public class database extends SQLiteOpenHelper {
    ...
    public int checkCart(String username,String product){
        ...
        SQLiteDatabase db=getReadableDatabase();
        Cursor c=db.rawQuery("Select * from cart where username=? and password=?",str);
        if(c.moveToFirst()){
            result=1;
        }
        db.close();
...
public ArrayList getOrderDetails(String username){
        ...
        if(c.moveToFirst()){
            do{
                arr.add(c.getString(1)+"S"+c.getString(2)+"$"+c.getString(3)+"$"+c.getString(4)+"$"+c.getString(5)+"$"+c.getString(6)+"$"+c.getString(7)+"$"+c.getString(8));
